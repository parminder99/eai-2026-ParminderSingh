...
const RABBITMQ_URL =
  ...
  "amqp://eai:eai-pa2@localhost:5673";

const QUEUE_NAME =
  process.env.QUEUE_NAME ?? "pa2.orders";
...
async function connectWithRetry(): Promise<Channel> {
  for (;;) {
    try {
      const channel = await connectConsumerChannel(
        RABBITMQ_URL,
        QUEUE_NAME,
      );
      ...
      console.error(
        "[consumer] connect failed, retrying in 2s: " +
...
/**
 * Open a channel onto the SAME durable queue the producer declares.
 * Also set a small prefetch — this consumer acks manually, and without
 * a prefetch limit RabbitMQ will happily hand it every message in the
 * queue at once.
 */
export async function connectConsumerChannel(
  url: string,
  queueName: string,
): Promise<Channel> {
...
async function run(): Promise<void> {
  ...
  for (;;) {
    ...
    const disconnected = new Promise<void>((resolve) => {
      ...
      connection.on("close", () => {
        console.error(
          "[consumer] connection closed — will reconnect",
        );
    ...
    await channel.consume(
      ...
      (msg) => {
        ...
        handleMessage(channel, msg).catch(
          (err: unknown) => {
            console.error(
              "[consumer] handleMessage failed: " +
                (err as Error).message,
            );
            ...
            try {
              ...
              /*
               * channel may already be closed if this
               * fired during a disconnect
               */
            }
          },
        );
...
if (
  process.argv[1] &&
  process.argv[1].endsWith("consumer.ts")
) {
  main();
}